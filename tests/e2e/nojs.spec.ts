...
test.describe('no JavaScript', () => {
  ...
  test('all essential homepage content remains visible and linked', async ({ page }) => {
    ...
    // The footer mailto CTA is gone in both modes; without JS the nav email
    // fallback (asserted below) is the contact path.
  ...
  test('resume and public archive remain readable without JavaScript', async ({ page }) => {
    ...
    for (const [index, disclosure] of (await disclosures.all()).entries()) {
      ...
      await expect(disclosure.locator('.media-shot')).toHaveCount(index === 0 ? 4 : 2);
