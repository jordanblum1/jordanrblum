...
export function scatter(count: number, seed: number): Placement[] {
  ...
  for (let i = 0; i < count; i++) {
    for (let attempt = 0; attempt < 40; attempt++) {
      ...
      const radius = 14 + 20 * Math.sqrt((i + rnd()) / count);
      const xPct = Math.min(92, Math.max(8, 50 + Math.cos(angle) * radius * 1.35));
      const yPct = Math.min(88, Math.max(12, 50 + Math.sin(angle) * radius));
